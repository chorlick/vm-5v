(function() {
  ...
  class CacheDatabase {
    ...
    list() {
      return this.scope.caches
        .keys()
        .then(keys => keys.filter(key => key.startsWith('ngsw:db:')));
    }
    open(name) {
      if (!this.tables.has(name)) {
  ...
  class AssetGroup {
    constructor(scope, adapter, idle, config, hashes, db, prefix) {
      ...
      this.origin = this.adapter.parseUrl(
    ...
    fetchAndCacheOnce(req, used = true) {
      return __awaiter(this, void 0, void 0, function*() {
        ...
        try {
          ...
          if (!res.ok) {
            throw new Error(
              `Response not Ok (fetchAndCacheOnce): request for ${
                req.url
              } returned response ${res.status} ${res.statusText}`
            );
          }
          try {
            ...
            if (!cacheBustedResult.ok) {
              throw new SwCriticalError(
                `Response not Ok (cacheBustedFetchFromNetwork): cache busted request for ${
                  req.url
                } returned response ${cacheBustedResult.status} ${
...
${msgIdle}`,
    ...
    formatDebugLog(log) {
      return log
        .map(
          entry => `[${this.since(entry.time)}] ${entry.value} ${entry.context}`
        )
        .join('\n');
    }
  }
    ...
    initialize() {
      return __awaiter$5(this, void 0, void 0, function*() {
        ...
        if (!this.versions.has(latest.latest)) {
          throw new Error(
            `Invariant violated (initialize): latest hash ${
              latest.latest
    ...
    setupUpdate(manifest, hash) {
      return __awaiter$5(this, void 0, void 0, function*() {
        ...
        if (manifest.configVersion !== SUPPORTED_CONFIG_VERSION) {
          ...
          yield this.scope.registration.unregister();
          throw new Error(
            `Invalid config version: expected ${SUPPORTED_CONFIG_VERSION}, got ${
    ...
    cleanupOldSwCaches() {
      return __awaiter$5(this, void 0, void 0, function*() {
        ...
        const oldSwCacheNames = cacheNames.filter(name =>
          /^ngsw:(?:active|staged|manifest:.+)$/.test(name)
        );
        yield Promise.all(
